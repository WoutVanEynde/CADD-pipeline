...
for dir in */; do
    ...
    obabel *.sdf -O $MOL2_OUT			# mol2 file is needed because GOLD cannot read sdf file, but sdf file is needed for splitting; otherwise dummy atoms used which is seen in gold.err file
    sleep 15 
    $GOLD_AUTO gold.conf &
    sleep 15
    cd ..
