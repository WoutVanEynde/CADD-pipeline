...
mypdbfile <- "/home/wout/PhD/MTERF3/MD/2_run/md_mmpbsa.pdb"
...
mydcdfile <- "/home/wout/PhD/MTERF3/MD/2_run/md_mmpbsa.dcd"
...
plot(rd, typ="l", ylab="RMSD (Å)", xlab="Frame No.", main="RMSD")
points(lowess(rd), typ="l", col="red", lty=2, lwd=2)

#Check RMSF of CA:
rf <- rmsf(xyz[,ca.inds$xyz])
plot(rf, ylab="RMSF (Å)", xlab="Residue Position", typ="l", main="RMSF")

#Principal Component Analysis of CA (PCA = dimension reduction method):
set.seed(1)
pc <- pca.xyz(xyz[,ca.inds$xyz], use.svd = TRUE)
plot(pc, col=bwr.colors(nrow(xyz)))
colorlegend(col= bwr.colors(nrow(xyz)), zlim = range(0:100), main="Timestep (ns)", posx = c(0.49, 0.51), posy = c(0.25, 0.75))
...
clust <- find.clusters(pc$z, n.pca = 100, choose.n.clust= FALSE)
print(clust$Kstat[1:50])
plot(clust$Kstat[1:10], type="b", col="blue", xlab = "", ylab = "")
title(xlab="Number of Clusters", ylab= "BIC", main = "Clusters")
...
hc3 <- hclust(dist(pc$z[,1:12]), method = "average") # amount of principal components used
grps3 <- cutree(hc3, k=2) #take k as amount of clusters
table(grps3)
plot(pc, col = grps3)
...
#cluster5 <- data.frame(pc.z[grps3==5,])
#cluster6 <- data.frame(pc.z[grps3==6,])
#cluster7 <- data.frame(pc.z[grps3==7,])
...
#Chose the first distance frame if the cosine is is smaller than 0.9:
"
Smallest distance frame		Smallest angle 		frame cosine 
44.0000000 			          45.0000000  		  0.9344525
"

#Select the frames in vmd and count +1 in the frame (in case frame 0 and 1 are the same):
